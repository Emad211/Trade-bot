...
def test_three_millisecond_funding_grid_jitter_is_recorded() -> None:
    spec_value = spec("FUNDING", 3)
    stream = io.StringIO(newline="")
    writer = csv.writer(stream, lineterminator="\n")
    writer.writerow([bp.START, 8, "0.0001"])
    writer.writerow([bp.START + 28_800_003, 8, "0.0001"])
    ...
    assert profile.exact_cadence_deviation_count == 1
    assert profile.max_abs_cadence_jitter_ms == 3


def test_four_millisecond_funding_grid_jitter_is_rejected() -> None:
    spec_value = spec("FUNDING", 2)
    stream = io.StringIO(newline="")
    writer = csv.writer(stream, lineterminator="\n")
    writer.writerow([bp.START, 8, "0.0001"])
    writer.writerow([bp.START + 28_800_004, 8, "0.0001"])
    raw = z("x.csv", stream.getvalue().encode())
    with pytest.raises(bp.PilotError, match="material funding grid jitter"):
