...
PENDING_FORMAL_LICENSE_INTERPRETATION

Historical settlement availability timing:
NOT VERIFIED

Canonical price linkage:
NOT AUTHORIZED
...
Pull-request merge-test commit:
...
- contract identities, dates, and schema passed;
- settlement-change reconciliation passed;
...
The evidence ZIP contains one JSON file only. Independent inspection confirmed that neither artifact contains CSV, HTML, a `VX_*` file, or derived price rows.
...
This is not raw immutable storage.
...
1. internal-research and retention rights;
2. a compliant private immutable storage contract if permitted;
3. historical settlement publication and `available_at` semantics;
4. explicit CFTC-to-VX contract selection by decision date;
5. expiry and roll-boundary handling without roll-gap PnL;
6. a no-lookahead same-contract price-linkage proposal.

The result must be either:

```text
GO_PRIVATE_RETENTION_AND_TIMING_CONTRACT
```

or:

```text
BLOCKED_LICENSE_OR_TIMING
```

Neither result is an economic-edge pass.
